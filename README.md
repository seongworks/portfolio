...
## 2-3. 웹사이트 화면 캡처를 올릴 때

화면 캡처는 가로가 아주 길어서(2:1 이상) 기본 크기로 두면
**한 장이 한 줄을 다 차지**합니다. 아래 한 줄을 적으면 조금 작아져서
두 장씩 나란히 놓입니다.
...
- `images` 바로 위에 적으면 됩니다.
- 안 적으면 지금까지처럼 원래 크기로 나옵니다.
- 이모티콘처럼 개수가 많은 작업에는 `gallery: "tiles"` 를 씁니다.
...
├── about.html          소개
├── contact.html        연락처
├── print.html          ★ A4 인쇄용 (PDF 로 뽑는 페이지)
│
├── data/
│   ├── site.js         ★ 이름 · 이메일 · SNS
│   └── projects.js     ★ 작업 목록
...
`print.html` 을 열고 **Ctrl + P** (맥은 Cmd + P) 를 누르면 됩니다.
...
| 용지 | **A4** |
| 여백 | **없음** |
| 배경 그래픽 | **켬** ← 이걸 안 켜면 사진이 안 나옵니다 |

- 구성: **표지 1장 + 작업 목록 1장 + 작업마다 1장**
- `data/projects.js` 를 그대로 읽으므로, **작업을 추가하면 PDF 도 자동으로 늘어납니다.**
- 사이트(검은 바탕)와 달리 **흰 바탕 · 검은 글씨**입니다. 종이에 뽑을 때 잉크를 덜 먹습니다.
- 사진은 **한 줄의 가로 합이 지면 폭과 딱 맞도록** 높이를 계산해서 깝니다.
  자리가 남으면 키우고, 넘치면 줄이므로 쪽마다 알아서 채워집니다.
  그래도 넘치면 `F12` → `Console` 에 몇 쪽이 넘쳤는지 한글로 알려줍니다.
- 설명은 쪽 아래에 붙습니다. 사진과 글 사이의 빈 자리는 일부러 둔 것입니다.

> 💡 한 작업에 사진이 너무 많으면 한 장에 다 넣느라 작아집니다.
> 그럴 때는 `projects.js` 의 `images` 에서 중요한 것만 남기면 커집니다.
